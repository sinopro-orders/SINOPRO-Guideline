...
새 컴프레서에 **동관 충전 밸브(Service Valve)** 를 먼저 브레이징합니다.
...
작업자는 가능한 한 **컴프레서의 오른쪽**에 위치하여 작업합니다.

* 한 손에는 브레이징 토치를 잡고
* 다른 한 손에는 브레이징 용접봉을 잡은 상태에서 작업을 진행합니다.
{% endstep %}

{% step %}
## 브레이징 부위 냉각

브레이징 작업이 완료되면 용접 부위에 물을 분사하여 냉각합니다.

{% hint style="warning" %}
**주의사항** — 물을 분사할 때에는 **브레이징 접합부(용접부)** 에 직접 물이 들어가지 않도록 주의하십시오.&#x20;
...
브레이징 상태를 다시 확인합니다. 다음 사항을 중점적으로 점검합니다.
...
컴프레서를 교체한 후에는 반드시 브레이징 상태와 냉매 누설 여부를 확인해야 합니다. 이를 위해 **매니폴드 게이지**를 연결하여 점검을 진행합니다.
...
브레이징 부위와 동관 연결부에 **비눗물**을 도포하여 기포가 발생하는지 확인합니다. 기포가 발생하면 해당 부위에 냉매 누설이 있는 것입니다.
...
② 기존 배선 순서에 맞춰 배선을 연결합니다. 기존 배선과 동일한 위치에 연결한 후, 새 부품의 단자에 배선을 브레이징하여 고정합니다.
...
이제 냉매를 정식으로 충전합니다. 먼저 진공펌프에 연결되어 있던 **노란색 호스**를 분리한 후, 냉매 용기에 다시 연결합니다.
{% endstep %}
